...
def solve(data):
    ...
    for line in data.splitlines():
        ...
        n = int(line[:delim])
        line = line[delim + 2:]
        props = [(x, int(y)) for x, y in [p.split(': ') for p in line.split(', ')]]
        if all(map(lambda x: facts[x[0]] == x[1], props)):
            print('Part 1 =', n)
        if all(map(lambda x: part2_valid(x[0], x[1]), props)):
            print('Part 2 =', n)
